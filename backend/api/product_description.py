...
@router.put("/update/{id}", dependencies=[Depends(current_superuser)])
async def update_product_description(id: int, product_description: ProductDescriptionUpdateSchema, db: AsyncSession = Depends(get_async_session)):
    product_description_repo = ProductDescriptionRepo(db)
    content = product_description.model_dump()
    result = await product_description_repo.update_product_description(id, content)
    if result:
