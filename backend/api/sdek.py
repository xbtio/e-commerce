...
@router.post("/order")
async def create_order(name_of_recepient: str, phone_of_recepient: str, additional_num: str, comment: str,user: User = Depends(current_user), db: AsyncSession = Depends(get_async_session)):
    sdek_service = SdekService()
    address_service = AddressService(db)
    order_repo = OrderRepo(db)
    address_of_user = await address_service.get_address_by_user_id(user.id)
    result = await sdek_service.create_order(name_of_recepient=name_of_recepient, email=user.email, phone_of_recepient=phone_of_recepient, additional_num=additional_num,address_of_recepient=address_of_user, comment=comment)
    if result is not None:
